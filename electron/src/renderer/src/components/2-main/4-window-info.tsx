...
export function WindowInfo() {
    ...
    return (
        <div className="p-2 border-b bg-muted/10 flex gap-4 text-sm">
            <div><span className="font-semibold">Handle:</span> {activeWindow.handle}</div>
            <div><span className="font-semibold">Process:</span> {activeWindow.processName} ({activeWindow.processId})</div>
            <div><span className="font-semibold">Class:</span> {activeWindow.className}</div>
            <div className="flex-1 text-right truncate"><span className="font-semibold">Title:</span> {activeWindow.title}</div>
        </div>
    );
}
