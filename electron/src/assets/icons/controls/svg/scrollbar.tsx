...
export function SvgSymbolScrollbar() {
    return (
        <symbol id="scrollbar" viewBox="0 0 24 24">
              <defs>
                <style>{`
            .cls-1, .cls-2 {
                    fill: none;
...
export function SymbolScrollbar({ className, title, children, ...rest }: SVGAttributes<SVGSVGElement> & HTMLAttributes<SVGSVGElement>) {
    return (
        <svg className={classNames("fill-none stroke-current", className)} {...rest}>
            {title && <title>{title}</title>}
            {children}
            <use xlinkHref="#scrollbar" />
        </svg>
    );
}
